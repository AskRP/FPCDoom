...
begin
  ...
  begin
    if name = itoa(i) then
    begin
      sprintf(result, SAVEGAMEPATH + '\' + SAVEGAMEIWAD + '\' + SAVEGAMENAME + '%s.dsg', [name]);
      exit;
    end;
  end;
...
begin
  ...
  savebuffer := Z_Malloc(maxsize, PU_STATIC, nil);
...
begin
  ...
  begin
    sprintf(name, M_SaveFileName(SAVEGAMEPATH + '\' + SAVEGAMEIWAD + '\' + SAVEGAMENAME) + '%d.dsg', [savegameslot]);
  end;
...
function G_NeedsCompatibilityMode: boolean;
begin
  result := compatibilitymode or G_IsOldDemoPlaying;
end;
